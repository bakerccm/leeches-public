...
# LSU elevation
    ...
    LSU.plot.elev + aes(color = (AdultBodyMass_g > 10000))
    ggsave(here("figures","FigS5_LSU_elev_10kg.pdf"), width = 12, height = 6, useDingbats = FALSE)

    # same plot with domestic mammals to assist annotation
    LSU.plot.elev + aes(color = domestic)
    ggsave(here("figures","FigS5_LSU_elev_domestic.pdf"), width = 12, height = 6, useDingbats = FALSE)

    # same plot highlighting species whose 95% BCI excludes zero to assist annotation
    LSU.plot.elev + aes(color = `95% BCI excludes zero`)
    ggsave(here("figures","FigS5_LSU_elev_BCI.pdf"), width = 12, height = 6, useDingbats = FALSE)
...
# LSU reserve
    ...
    # same plot with 10kg mammals to assist annotation
    LSU.plot.reserve + aes(color = (AdultBodyMass_g > 10000))
    ggsave(here("figures","FigS6_LSU_reserve_10kg.pdf"), width = 12, height = 6, useDingbats = FALSE)

    # same plot with domestic mammals to assist annotation
    LSU.plot.reserve + aes(color = domestic)
    ggsave(here("figures","FigS6_LSU_reserve_domestic.pdf"), width = 12, height = 6, useDingbats = FALSE)

    # same plot highlighting species whose 95% BCI excludes zero to assist annotation
    LSU.plot.reserve + aes(color = `95% BCI excludes zero`)
    ggsave(here("figures","FigS6_LSU_reserve_BCI.pdf"), width = 12, height = 6, useDingbats = FALSE)

 # SSU elevation

    SSU.plot.elev <- beta.output$SSU$elev %>%
        mutate(`95% BCI excludes zero` = ifelse((`2.5%` > 0) | (`97.5%` < 0), TRUE, FALSE)) %>%
        inner_join(taxa, by = "OTU") %>% mutate(class = tolower(class)) %>%
        left_join(taxa.iucn %>% select(OTU, `IUCN category`), by = "OTU") %>%
        left_join(leech %>% select(OTU, AdultBodyMass_g, domestic) %>% distinct(), by = "OTU") %>%
