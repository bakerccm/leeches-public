# R code for generating community occupancy and detection predictions
# SSU dataset
...
# input and output file names
    ...
    filenames <- list(
        ...
        modeloutput.rds = here("rds", "Ailaoshan_model_output_200_final_SSU.rds"), # file containing modelling results

        output.rdata = here("rdata", "Ailaoshan_occupancy_covariates_SSU_community_predictions.rdata") # file name to save predictions to
...
# elevation
    ...
    for(c in 1:nchains){
        for(s in 1:nsamp){
            elev.pred[, s + (c - 1) * nsamp, 1] <- plogis(model.output$samples[[c]][s, "mu.eta[1,1]"] + model.output$samples[[c]][s, "mu.beta"] * elev.vals$scaled)  # psi ~ elev for group 1 (i.e. mammals/birds)
            elev.pred[, s + (c - 1) * nsamp, 2] <- plogis(model.output$samples[[c]][s, "mu.eta[1,2]"] + model.output$samples[[c]][s, "mu.beta"] * elev.vals$scaled)  # psi ~ elev for group 2 (i.e. amphibians/reptiles)
        }
    }
...
SSU.community.predictions <- list(
    occupancy.elevation = occupancy.elevation,
    detection = detection
)

save(SSU.community.predictions, file = filenames$output.rdata)
