# R code to draw correlation networks from the already-processed preOTU data
...
# get preOTU table data

    load(file=here("rdata","Ailaoshan_preOTUs.rdata"))

# calculate pairwise Spearman correlations within datasets
    ...
    for (i in c("LSU", "SSU")) {
        ...
        # calculate correlations based on observed presence/absence (i.e. detection = true/false)
...
# calculate pairwise Spearman correlations between datasets
...
# add best guess taxonomy information to read tables for each dataset
...
# add best guess taxon data to vertices
...
# color edges black if taxon best guesses match
...
# size vertices by (raw) occupancy
    ...
    # LSU, SSU graphs
        ...
        for (i in c("LSU", "SSU")) {
            occupancy[[i]] <- read.tidy[[i]] %>%
                group_by(OTU) %>%
                dplyr::summarize(occupancy = sum(reads > 0)/n(), .groups="drop")
...
# draw plots
    ...
    pdf(here("preOTU_networks","preOTU_network_LSU_weighted.pdf"), width=20, height=20, useDingbats = FALSE)
    ...
    pdf(here("preOTU_networks","preOTU_network_SSU_weighted.pdf"), width=20, height=20, useDingbats = FALSE)
    ...
    pdf(here("preOTU_networks","preOTU_network_bipartite_weighted.pdf"), width=20, height=20, useDingbats = FALSE)
