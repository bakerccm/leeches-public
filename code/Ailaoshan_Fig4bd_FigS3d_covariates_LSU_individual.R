...
# input and output file names
    ...
    filenames <- list(
        ...
        modeloutput.rds = here("rds", "Ailaoshan_model_output_200_final_LSU.rds"), # file containing LSU modelling results
...
# lists of empty arrays to fill with predictions
    ...
    predictions <- list(
        ...
        reserve = array(NA, dim = c(500, num.detected.species), dimnames = list(1:500, colnames(jags.data$z.start)[1:num.detected.species])),
...
# generate predictions
    for(k in 1:num.detected.species){
        # empty arrays to fill with predictions for species k
            ...
            reserve.pred <- array(NA, dim = c(nrow(pred.vals$reserve), nchains * nsamp)) # empty array to be filled
        ...
        # generate predictions based on the mcmc iterations
            for(c in 1:nchains){
                for(s in 1:nsamp){
                    ...
                    reserve.pred[, s + (c - 1) * nsamp] <- plogis(model.output$samples[[c]][s, paste0("beta0[", k, "]")] + model.output$samples[[c]][s, paste0("beta[2,", k, "]")] * pred.vals$reserve$scaled) # psi ~ reserve for species k
